<?php
/**
 * @author Ascensio System SIA
 */
$strings['plugin_title'] = 'ONLYOFFICE';
$strings['plugin_comment'] = 'Коннектор ONLYOFFICE позволяет просматривать и редактировать текстовые документы, электронные таблицы и презентации и работать над ними совместно в Chamilo, используя ONLYOFFICE Docs.';
$strings['enable_onlyoffice_plugin'] = 'Включить';
$strings['document_server_url'] = 'Адрес ONLYOFFICE Docs';
$strings['document_server_url_help'] = 'Здесь укажите адрес вашего сервера ONLYOFFICE Docs';
$strings['jwt_secret'] = 'Секретный ключ сервера документов';
$strings['jwt_secret_help'] = 'Оставьте поле пустым, чтобы отключить';
$strings['openByOnlyoffice'] = 'Открыть с помощью ONLYOFFICE';
$strings['createNew'] = 'Создать новый';
$strings['title'] = 'Заголовок';
$strings['chooseFileFormat'] = 'Выбрать формат файла';
$strings['document'] = 'Текстовый документ';
$strings['spreadsheet'] = 'Электронная таблица';
$strings['presentation'] = 'Презентация';
$strings['create'] = 'Создать';
$strings['fileIsExist'] = 'Файл уже существует';
$strings['impossibleCreateFile'] = 'Невозможно создать файл';
$strings['createNewDocument'] = 'Создать новый документ';
$strings['formTemplate'] = 'PDF форма';
$strings['fillInFormInOnlyoffice'] = 'Заполнить форму в ONLYOFFICE';
$strings['UpdateOnlyoffice'] = 'Обновите сервер ONLYOFFICE Docs до версии 7.0 для работы с формами онлайн';
$strings['NotSupportedVersion'] = 'Неподдерживаемая версия';
$strings['jwt_header'] = 'Заголовок авторизации';
$strings['jwt_header_help'] = 'Оставьте поле пустым, чтобы использовать заголовок по умолчанию';
$strings['connect_demo'] = 'Подключиться к демонстрационному серверу ONLYOFFICE Docs';
$strings['demoPeriodIsOver'] = '30-дневный тестовый период закончен, вы больше не можете подключаться к демонстрационному серверу ONLYOFFICE Docs.';
$strings['demoPrevMessage'] = 'Это публичный тестовый сервер, пожалуйста, не используйте его для личных конфиденциальных данных. Сервер будет доступен в течение 30 дней.';
$strings['demoUsingMessage'] = 'Вы используете публичный демонстрационный сервер ONLYOFFICE Docs. Пожалуйста, не храните конфиденциальные данные.';
$strings['document_server_internal'] = 'Адрес ONLYOFFICE Docs для внутренних запросов сервера';
$strings['storage_url'] = 'Адрес сервера для внутренних запросов ONLYOFFICE Docs';
$strings['DocsCloudBannerTitle'] = 'ONLYOFFICE Docs Cloud';
$strings['DocsCloudBannerMain'] = 'Легко запускайте редакторы в облаке без скачивания и установки';
$strings['DocsCloudBannerButton'] = 'Получить сейчас';
$strings['connectionError'] = 'При попытке соединения возникла ошибка';
$strings['docServiceError'] = 'Возникла ошибка в сервисе документов';
$strings['pluginIsNotConfigured'] = 'Приложение ONLYOFFICE не сконфигурировано. Пожалуйста, свяжитесь с администратором.';
$strings['cantReadXml'] = 'Невозможно прочитать xml файл на сервере';
$strings['badResponseErrors'] = 'Неправильный ответ. Ошибки: ';
$strings['mixedContent'] = 'Смешанное активное содержимое запрещено. Для ONLYOFFICE Docs необходимо использовать HTTPS-адрес.';
$strings['badHealthcheckStatus'] = 'Ошибка при проверке работоспособности';
$strings['UserVoiceTitle'] = 'Добро пожаловать в ONLYOFFICE Docs!';
$strings['UserVoiceDescription'] = 'Работайте над документами, электронными таблицами, презентациями и формами совместно в рамках структуры Chamilo, используя ONLYOFFICE Docs.';
$strings['LearnMore'] = 'Узнать больше';
$strings['SuggestFeature'] = 'Предложите функцию';
$strings['EmptyJwtWarning'] = 'Для обеспечения безопасности важных параметров в запросах ONLYOFFICE Docs, пожалуйста, установите секретный ключ на странице настроек.
Чтобы узнать больше, <a target="_blank" style="text-decoration:underline;" href="https://api.onlyoffice.com/docs/docs-api/get-started/how-it-works/security/">перейдите по ссылке</a>.';
$strings['OnlyofficeUnreachable'] = 'ONLYOFFICE недоступен. Пожалуйста, свяжитесь с администратором';
$strings['fileSavedAs'] = 'Файл сохранен как';
$strings['fileCreateForbidden'] = 'Создание файлов запрещено.';
$strings['docsUrlError'] = 'Не удалось подключиться к ONLYOFFICE Docs. Убедитесь, что сервер запущен и доступен.';
$strings['commandServiceError'] = 'Ошибка при попытке проверить CommandService';
$strings['convertServiceError'] = 'Ошибка при попытке проверить ConvertService';
$strings['docsUnknownError'] = 'При проверке соединения произошла непредвиденная ошибка';
$strings['invalidSecret'] = 'Не удалось подключиться к ONLYOFFICE Docs. Проверьте правильность секретного ключа.';
